...
(** (_Hint_: You can use [apply] with prevviously defined lemmas, not
...
Theorem rev_exercise1 : forall (l l' : list nat),
     ...
     l' = rev l.
Proof.
  intros list1 list2 H.
  symmetry.
  assert(rev list1 = rev (rev list2)).
...
(** Briefly explain the difference between the tactics [apply] and
    ...
    applied?
Rewrite is for equality, apply is for *implication*.

Rewrite allows one to go between two sides of the equality.


Apply on implication allows one to either:
* apply an implication of the form (a -> b) on a to yield b.
* Apply an implication of the form (a1 -> a2 -> a3 ... -> an -> b) on b to
yeild the hypothesis (a1, a2, ..., an).
(* FILL IN HERE *)
*)
(** [] *)
